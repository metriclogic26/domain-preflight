#!/usr/bin/env python3
"""Generate incidents/[slug]/index.html — TechArticle + FAQPage + BreadcrumbList."""
...
__LDJSON__
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .incident-meta { font-size: 0.78rem; color: var(--text2); margin-bottom: 1.75rem; }
    .incident-meta time { color: var(--purple); }
...
__BREADCRUMB__
    <article>
      ...
      <p class="incident-meta">Published <time datetime="__DATE__">__DATE_PRETTY__</time></p>
__ARTICLE__
    </article>
  </main>
  <footer>
...
def build_ldjson(
    ...
    parts = [
        ...
        + json.dumps(tech, ensure_ascii=False, separators=(",", ":"))
        + "\n  </script>",
        '  <script type="application/ld+json">\n  '
...
def emit(page: dict) -> None:
    slug = page["slug"]
    canonical = f"{HOME}incidents/{slug}/"
    h1 = page["h1"]
    crumb_name = page.get("breadcrumb_label", h1)
    trail = [
        ("Home", HOME),
        ("Incidents", f"{HOME}incidents/"),
        (crumb_name, canonical),
    ]
    crumbs = breadcrumb_schema(canonical, trail)
    ld = build_ldjson(canonical, h1, page["meta"], page["faqs"], crumbs)
    nav = [
        ("Home", HOME),
        ("Incidents", "/incidents/"),
        (page["breadcrumb_label"], None),
    ]
    body = page["body"] + "\n" + faq_section(page["faqs"]) + """
      <div class="internal-links">
        <p><a href="/incidents/">← All incidents</a></p>
    ...
    html = html.replace("__HERO__", page.get("hero", "Incident"))
...
def main() -> None:
    ...
    from incident_pages_data import INCIDENTS  # noqa: PLC0415

#!/usr/bin/env python3
"""Generate learn/[slug]/index.html — Article + optional HowTo + FAQPage + BreadcrumbList."""
...
__LDJSON__
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-size: clamp(1.35rem, 3.5vw, 1.9rem); font-weight: 600; margin-bottom: 0.75rem; line-height: 1.3; }
    .subtitle { color: var(--text2); font-size: 0.95rem; margin-bottom: 0.5rem; }
    .learn-meta { font-size: 0.78rem; color: var(--text2); margin-bottom: 1.75rem; }
    .learn-meta time { color: var(--purple); }
    article h2 { font-size: 1.05rem; font-weight: 600; margin: 2rem 0 0.75rem; color: var(--text); }
    article h3 { font-size: 0.95rem; font-weight: 600; margin: 1.25rem 0 0.5rem; color: var(--text); }
...
__BREADCRUMB__
    <article>
      ...
      <p class="learn-meta">Updated <time datetime="__DATE__">__DATE_PRETTY__</time></p>
__ARTICLE__
    </article>
  </main>
  <footer>
...
def build_howto_json(
    ...
    return {
        ...
        "supply": {"@type": "HowToSupply", "name": "DNS and email admin access"},
...
def build_ldjson(
    ...
    article = {
        ...
        "publisher": {
            ...
            "logo": {
                "@type": "ImageObject",
                "url": "https://domainpreflight.dev/favicon.png",
            },
        },
    ...
    crumbs = {"@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": crumb_items}
    parts = [
...
def emit(page: dict) -> None:
    slug = page["slug"]
    canonical = f"{HOME}learn/{slug}/"
    h1 = page["h1"]
    crumb_name = page.get("breadcrumb_label", h1)
    trail = [
        ("Home", HOME),
        ("Learn", f"{HOME}learn/"),
        (crumb_name, canonical),
    ]
    crumbs = breadcrumb_schema(canonical, trail)
    howto = page.get("howto")
    ld = build_ldjson(canonical, h1, page["meta"], page["faqs"], crumbs, howto)
    nav = [
        ("Home", HOME),
        ("Learn", "/learn/"),
        (crumb_name, None),
    ...
    body = page["body"]
    # Visible .howto-step divs must match HowTo JSON; append unless embedded in body.
    if howto and not page.get("howto_steps_embedded"):
        body = body + "\n" + howto_steps_html(howto["step_texts"])

    body = (
        body
        + "\n"
        + faq_section(page["faqs"])
        + """
      <div class="internal-links">
        <p><a href="/learn/">← All learning guides</a></p>
        <p><a href="https://domainpreflight.dev/">Open DomainPreflight →</a></p>
      </div>"""
    )
    html = STYLE.replace("__TITLE__", page["title"])
    html = html.replace("__META__", page["meta"])
    html = html.replace("__CANON__", canonical)
    html = html.replace("__LDJSON__", ld)
    html = html.replace("__BREADCRUMB__", breadcrumb_nav(nav))
    html = html.replace("__HERO__", page.get("hero", "Guide"))
    html = html.replace("__H1__", h1)
    html = html.replace("__SUB__", page["subtitle"])
    html = html.replace("__DATE__", DATE_PUBLISHED)
    html = html.replace("__DATE_PRETTY__", "March 19, 2026")
    html = html.replace("__ARTICLE__", body)
    out = LEARN / slug / "index.html"
...
def main() -> None:
    ...
    from learn_pages_data import GUIDES  # noqa: PLC0415
